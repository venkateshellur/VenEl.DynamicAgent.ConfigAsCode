...
{
    private readonly Dictionary<string, Func<AgentConfig, ILlmClient, IAgentLogger, IAgent>> _agentFactories 
    ...
    public void RegisterAgentType(string type, Func<AgentConfig, ILlmClient, IAgentLogger, IAgent> factory)
    {
    ...
    /// </summary>
    public IAgent CreateAgent(AgentConfig config, ILlmClient llmClient, IAgentLogger logger)
    {
        if (_agentFactories.TryGetValue(config.Type, out var factory))
        {
            return factory(config, llmClient, logger);
        }
